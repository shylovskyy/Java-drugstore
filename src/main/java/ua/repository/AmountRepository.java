...
public interface AmountRepository extends JpaRepository<Amount, Long>, JpaSpecificationExecutor<Amount>{
	...
	@Query("SELECT a FROM Amount a WHERE a.amount=?1 and a.name=?2  and  a.recipe=?3 and a.ingredient.id=?4 and a.system.id=?5")
	Amount findUnique(BigDecimal amount, String name, String recipe,  Long ingredientId, Long systemId);
