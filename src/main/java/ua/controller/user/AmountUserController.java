...
@Controller
@RequestMapping("/user/amountuser")
@SessionAttributes(names="amount")
public class AmountUserController {
	...
	public String show(Model model,
			...
			@ModelAttribute("filter") AmountFilter filter){
	...
	public String save(@ModelAttribute("amount") @Valid AmountForm amount, BindingResult br, Model model, SessionStatus status, @PageableDefault Pageable pageable, @ModelAttribute("filter") AmountFilter filter){
		...
		status.setComplete();
		return "redirect:/user/amountuser"+getParams(pageable, filter);
	}
